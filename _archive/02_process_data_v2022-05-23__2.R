...
process_data_version_id <- "v2022-05-23"
...
inputdata_config_path <- c(
  "/mnt/workspace/GWTG/COVID19/ihme_diagnostics/thrombosis/working_dirs/v2022-05-23/config.RDS"
)
...
if (0) {
  # reload input data?
  cmd <- paste(
    paste0("/opt/R/4.0.2/lib/R/bin/Rscript ", file.path(code_dir, "01_load_data_v2022-05-23.R")),
...
df2$during_anticoag_dvt_type <- ifelse(is.na(df2$during_anticoag_dvt_type), "Not applicable", df2$during_anticoag_dvt_type)
if (0) {
...
for (var in anticoag_vars) {
  ...
  if (var == "during_anticoag_dvt_type") {
    df2_anticoag_tmp[newvar_name] <-  !df2[, var] %in% "Not applicable" & is.na(df2[, paste0(var, "_date")])
  } else {
