#
# 01_process_data_v2022-05-23.R 
#
# March 2022
#
# Note: If updating the date, need to change it in three places below (currently lines 16, 22, and 42)
...
process_data_version_id <- "v2022-05-23__5"
...
inputdata_config_path <- c(
  "/mnt/workspace/GWTG/COVID19/ihme_diagnostics/thrombosis/working_dirs/v2022-05-23__5/config.RDS"
)
...
if (0) {
  # reload input data?
  cmd <- paste(
    paste0("/opt/R/4.0.2/lib/R/bin/Rscript ", file.path(code_dir, "01_load_data_v2022-05-23__5.R")),
...
# -- reclassify the during_anticoag_dvt_type variable, 
#    adding it to the existing variables

# Email from Mat on May 23, 2022:
# Transformation:
#   ANTICOAGDVT_1 (full dose doac) = during_DOAC 
#   ANTICOAGDVT_2 (full dose enoxaparin) = During_subq_lmwh_full 
#   ANTICOAGDVT_4 (low dose enoxaparin) = During_subq_lmwh_low   
# 
# Imputation: 
#   if ANTICOAGDVT_3 (low dose DOAC) = missing, then = 0


df2$during_anticoag_dvt_type_date <- df2$during_anticoag_dvt_date
df2$during_anticoag_dvt_type <- ifelse(is.na(df2$during_anticoag_dvt_type), "Not applicable but not missing either", df2$during_anticoag_dvt_type)
if (0) {
  table(df2$during_anticoag_dvt_type, useNA = "always")
  table(df2$during_anticoag_dvt, useNA = "always")
}
...
anticoag_date_vars <- c(
  ...
  "during_anticoag_dvt_type_date"
)


# -- remove patients that have any 'yes' to anticoagulation but missing a date
...
for (var in anticoag_vars) {
  ...
  if (var == "during_anticoag_dvt_type") {
    df2_anticoag_tmp[newvar_name] <-  !df2[, var] %in% "Not applicable but not missing either" & is.na(df2[, paste0(var, "_date")])
  } else {
...
if (0) {
  df_tmp <- df2 %>%
    as.data.frame(.) %>%
    .[, c("admission_date", anticoag_date_vars, gsub("_date", "", anticoag_date_vars))]
  
  with(df_tmp %>% filter(!is.na(during_anticoag_dvt)), hist(admission_date, breaks = 30, main = "Admission dates among non-missing 'ANTCOGDVTPRO'"))
  with(df_tmp %>% filter(!is.na(during_anticoag_dvt_type)), hist(admission_date, breaks = 30, main = "Admission dates among non-missing 'ANTICOAGDVT'"))
  with(df_tmp %>% filter(!is.na(during_subq_lmwh_low)), hist(admission_date, breaks = 30, main = "Admission dates among non-missing 'SUBQLMWHLWDOS'"))
}

sapply(anticoag_vars, function(x) table(df2[, x], useNA = "always"), simplify = FALSE)
...
if (0) {
  table(df2$n_missing_anticoag_val, useNA = "always")
}
